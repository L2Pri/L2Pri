...
def get_ckptdir(net_name, net_structure, tmp_fold, sigma=None,):
    data_fold = tmp_fold
    net_name = '{}-{}'.format(net_name, '-'.join([str(x) for x in net_structure]))
    if sigma:
        net_name += '-scale-{}'.format(sigma)
    ckptdir = os.path.join(os.path.dirname(__file__), 'ckptdir', data_fold)
...
def load_train_vali_data(data_fold, small_dataset=False):
    ...
    if small_dataset:
        ...
        train_file, valid_file = "train.dat","test.dat"
    else:
        #train_file, valid_file = "train.txt", "vali.txt"
        train_file, valid_file = "train.dat","test.dat"

    #data_dir = 'data/mslr-web10k/'
    data_dir = '/devdata3/zjy/L2RTCP/code/data/cross_data_py/'
    ...
    #valid_data = os.path.join(os.path.dirname(__file__), data_dir, valid_file)
...
def get_args_parser():
    ...
    parser.add_argument("--additional_epoch", dest="additional_epoch", type=int, default=300)
    ...
    parser.add_argument("--data_fold", dest="data_fold", type=str)
    return parser
