...
root_path = '/devdata3/zjy/L2RTCP/code/data/cross_data_py/'
...
def writeResult(filepath,content):
    f = open(filepath,'w')
...
#############################
def train_rank_net(
    start_epoch=0, additional_epoch=5, lr=0.0001, optim="adam",
    ...
    net, net_inference, ckptfile = get_train_inference_net(
        train_algo, train_loader.num_features, start_epoch, double_precision, data_fold
    )
    device = get_device()
    net.to(device)
    ...
    qid_result,res_result,prediction_result = eval_output(
            net_inference, device, df_valid, valid_loader, 1000000, [10, 30], start_epoch + additional_epoch, writer)


    writeResult('%s/%s/RankNet_pred.dat'%(root_path,data_fold),prediction_result)
    #assert(0)
...
def get_train_inference_net(train_algo, num_features, start_epoch, double_precision, data_fold):
    ...
    if train_algo == BASELINE:
        ...
        ckptfile = get_ckptdir('ranknet', ranknet_structure, data_fold)

    elif train_algo in [SUM_SESSION, ACC_GRADIENT]:
        net = RankNet(ranknet_structure, double_precision)
        net_inference = net
        ckptfile = get_ckptdir('ranknet-factorize', ranknet_structure, data_fold)
...
def eval_model(inference_model, device, df_valid, valid_loader, epoch, writer=None):
    ...
    with torch.no_grad():
        ...
        #pre_result = inference_model(valid_loader)
    return ndcg_result
