...
def ARP(test_list,cov_list,time_list):
    length = len(cov_list)
    ...
    for i in range(length):
        for j in range(i,length):
            ...
            DistanceMetric[i][j] = temp_distance/(time_list[i] * time_list[j] * 1.0)
            DistanceMetric[j][i] = temp_distance/(time_list[i] * time_list[j] * 1.0)
...
if __name__ == '__main__':
    ...
    for subject in subject_list:
        ...
        for training_index in range(1):
            st = time.time()
            arp = ARP(testlist_ori,coveragelist_ori,timelist_ori)
            ...
            f = open(subject_path + 'training/TARTStatement','w')
            for item in arp:
                f.write(str(item) + '\n')
            f.close()
            f = open(subject_path + 'training/TimeTARTStatement','w')
