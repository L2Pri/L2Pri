...
if __name__ == '__main__':
    ...
    for subject in subject_list:
        ...
        for training_index in range(1):
            st = time.time()
            gt = timeonly(testlist_ori,timelist_ori)
            prioritize_time = time.time() - st
            createFolder(subject_path + 'training/')
            f = open(subject_path + 'training/TOStatement','w')
            for item in gt:
                f.write(str(item) + '\n')
            f.close()
            f = open(subject_path + 'training/TimeTOStatement','w')
