...
def getJaccardDistance(a,b):
    ...
    join_count = join.count('1')
    combine_count = combine.count('1')
    #join_count = 0
    #combine_count = 0
    #jdst = time.time()
    '''
...
if __name__ == '__main__':
    ...
    for subject in subject_list:
        ...
        for training_index in range(1):
            #index_list = readFile(subject_path + 'testList')
            #testlist = []
            #coveragelist = []
            #for index in index_list:
            #    temp_index = eval(index)
            #    testlist.append(testlist_ori[temp_index])
            #    coveragelist.append(coveragelist_ori[temp_index])
            st = time.time()
            arp = ARP(testlist_ori,coveragelist_ori)
            ...
            f = open(subject_path + 'training/ARTStatement','w')
            for item in arp:
                f.write(str(item) + '\n')
            f.close()
            f = open(subject_path + 'training/TimeARTStatement','w')
            f.write(str(prioritize_time))
            f.close()
            #print('%s - %s is completed!'%(subject,training_index))
