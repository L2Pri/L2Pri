...
public class TimeGenetic {
	...
	public TimeGenetic(String Directory, char[][] trainingCovMaxtrix, float[] trainingtime){
	...
	// get the sum of timelist.
	public void getTimesum(float[] TimeList){
		this.timesum = 0;
		for (int i = 0; i<TimeList.length;i++){
			this.timesum = this.timesum + TimeList[i];
		}
	}


	//Read the Coverage File and Store the value to the APBC, APDC or APSC Matrix.
	public float getCoverageMatrix(char[][] CoverageMatrix){
		float metric = 0;
		
		try{
			int columnNum = CoverageMatrix[0].length;
			//this.CoverageMatrix = new char[tempAl.size()][columnNum]; //Initialize the Coverage Matrix.
			this.GenesNum = CoverageMatrix.length; //Initialize the GenesNum, which is the Test Case Number.
			...
			this.CoveredNum = testCaseCovered.size();
			/*Check
	...
	public float getAveragePercentageCoverage(int[] Chromosome){
		float AveragePercentageCoverageC = 0;
		float firstCoveredSum = 0; //Sum of the first test case in the order that covers the Block/Statement/Branch i.
			//System.out.println("testCaseCovered Size: "+this.testCaseCovered.size());
		for(int k=0; k<this.testCaseCovered.size(); k++){
			float tmpcount = 0;
			for(int i=0; i<Chromosome.length; i++){
				tmpcount = tmpcount + this.TimeList[i];
				if(this.CoverageMatrix[Chromosome[i]][this.testCaseCovered.get(k)] == '1'){
					//firstCoveredSum += i;
					firstCoveredSum = firstCoveredSum + (this.timesum - tmpcount + (float)0.5 * this.TimeList[i]);
					break;
				}
			}
		}
		//System.out.println("firstCoveredSum: "+firstCoveredSum);
		//AveragePercentageCoverageC = 1-(float)((float)firstCoveredSum / (float)(this.GenesNum * this.testCaseCovered.size()))+(1.0F / (float)(2 * this.GenesNum));
		AveragePercentageCoverageC = firstCoveredSum/(this.timesum * this.testCaseCovered.size());
		return AveragePercentageCoverageC;
	}
	...
	public int[] StartGeneration(){
		...
		this.getCoverageMatrix(this.CoverageMatrix); //Read the Coverage Matrix.
		this.getTimesum(this.TimeList);
