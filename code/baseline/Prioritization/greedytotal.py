...
def quick_sort(temp_name, temp_cov):
    temp_list = []
...
def greedytotal(test_name,test_cov):
    #test_cov_index = str2int(test_cov_index)
    selectedTestSequence = quick_sort(test_name,test_cov)
    result_list = []
...
if __name__ == '__main__':
    ...
    for subject in subject_list:
        ...
        for training_index in range(1):
            st = time.time()
            gt = greedytotal(testlist_ori,coveragelist_ori)
            prioritize_time = time.time() - st
            createFolder(subject_path + 'training/')
            f = open(subject_path + 'training/GTStatement','w')
            for item in gt:
                f.write(str(item) + '\n')
            f.close()
            f = open(subject_path + 'training/TimeGTStatement','w')
